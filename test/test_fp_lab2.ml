...
let to_list bag =
  List.rev (fold_left (fun acc x -> x :: acc) [] bag)
...
let test_remove_nonexistent () =
  ...
  Alcotest.(check int_list) "unchanged after remove nonexistent" (to_list bag) (to_list bag')

let test_remove_all () =
  let bag = empty |> insert 5 |> insert 5 |> insert 5 |> insert 3 in
  let bag' = remove_all 5 bag in
  Alcotest.(check int) "no occurrences after remove_all" 0 (count_occurrences 5 bag');
...
(* filter тесты *)
let test_filter () =
  let bag = empty |> insert 1 |> insert 2 |> insert 3 |> insert 4 |> insert 5 |> insert 6 in
...
module PBT = struct
  ...
  let bag_gen = Gen.(list_size (int_bound 10) (int_bound 20) |> 
  ...
  let equal a b = 
    let to_list bag = List.sort compare (fold_left (fun acc x -> x::acc) [] bag) in
    to_list a = to_list b

  let qcheck_tests = [
    Test.make ~name:"left identity" bag_arb 
      (fun a -> equal a (union empty a));
    
    Test.make ~name:"right identity" bag_arb 
      (fun a -> equal a (union a empty));
    
    Test.make ~name:"associativity" triple_arb 
      (fun (a,b,c) -> equal (union (union a b) c) (union a (union b c)));
    
    Test.make ~name:"commutativity" pair_arb 
      (fun (a,b) -> equal (union a b) (union b a));
  ]
